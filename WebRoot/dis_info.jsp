...
<html>
  <head>
	...
	-->
		  ...
		  <script type="text/javascript">
    	function Up_date(EMPNO,ENAME,JOB,MGR,HIREDATE,SAL,COMM,DEPTNO,STATUS)
    	{		
    		
    		var url ="Pre_update.action?EMPNO="+EMPNO+"&ENAME="+encodeURI(encodeURI(ENAME))+"&JOB="+encodeURI(encodeURI(JOB))+"&MGR="+MGR+"&HIREDATE="+HIREDATE+"&SAL="+SAL+"&COMM="+COMM+"&DEPTNO="+DEPTNO+"&STATUS="+encodeURI(encodeURI(STATUS));
    		location.href=url;
...
<body class=" theme-blue">
	...
	<form action="Load.action" class="form-inline" style="margin-top:0px;">
                    <input name = "test" value="<s:property value="#request.test"/>" class="input-xlarge form-control" placeholder="输入编号或名字..." id="appendedInputButton" type="text">
...
<div class="row">
        ...
        <div class="panel panel-default">
            <div class="panel-heading no-collapse">员工信息<span class="label label-warning"></span></div>
            <table class="table table-bordered table-striped" >
   			 <thead>
   				<tr>
   					<td>员工编号</td>  <td>姓名</td>	<td>职位</td>	<td>领导编号</td>	<td>雇佣日期</td>	<td>薪资</td>	<td>奖金</td>	<td>部门编号</td>
   					<td>目前状态</td>
   				</tr>
   				 </thead>
              <tbody>
   				...
   				<tr>
					<td><s:property value="#li.EMPNO"/> </td> 
					<td><s:property value="#li.ENAME"/> </td> 
					<td><s:property value="#li.JOB"/> </td> 
					<td><s:property value="#li.MGR"/> </td> 
					<td><s:property value="#li.HIREDATE"/> </td>   
					<td><s:property value="#li.SAL"/> </td>  
					<td><s:property value="#li.COMM"/> </td>  
					<td><s:property value="#li.DEPTNO"/> </td>  	
					<td><s:property value="#li.STATUS"/> </td>  
					<td><input class="btn btn-primary pull-right" type="button" id="submit" value="修改" onclick="Up_date(<s:property value="#li.EMPNO"/>,
					'<s:property value="#li.ENAME"/>',
					'<s:property value="#li.JOB"/>' ,
					<s:property value="#li.MGR"/>,
					'<s:property value="#li.HIREDATE"/>',
					'<s:property value="#li.SAL"/>',
					'<s:property value="#li.COMM"/>',
					'<s:property value="#li.DEPTNO"/> ',
					'<s:property value="#li.STATUS"/>')"/></td>			
   				</tr>
              <tbody>
   			</s:iterator>
   			</table>
